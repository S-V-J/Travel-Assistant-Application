...
def init_query_db(db_path: str = "/home/stjl0/livekit-travel-voice-assistant/db/query_history.db"):
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
            ...
            # Add query_count column if it doesn't exist
            if not check_query_count_column(db_path):
                cursor.execute("ALTER TABLE query_history ADD COLUMN query_count INTEGER DEFAULT 1")
            conn.commit()
...
2. get_flights: {"from_location": "string", "to_location": "string"}
...
def check_cache(query: str, db_path: str = "/home/stjl0/livekit-travel-voice-assistant/db/query_history.db", max_age_hours: int = 24, similarity_threshold: int = 90):
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
...
def store_query_response(query: str, response: str, tool_name: str, db_path: str = "/home/stjl0/livekit-travel-voice-assistant/db/query_history.db"):
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
...
def cleanup_old_entries(db_path: str = "/home/stjl0/livekit-travel-voice-assistant/db/query_history.db", max_age_days: int = 30):
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
